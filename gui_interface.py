...
def proceed():
    ...
    if pdf_path is not None:
        ...
        text_file_path = main(pdf_path)  # Capture the returned text file path
        root.destroy()  # This will destroy the Tkinter window

        # Open the generated text file with the default application based on the OS
        if platform.system() == 'Windows':
            os.startfile(text_file_path)
        elif platform.system() == 'Darwin':  # macOS
            subprocess.run(['open', text_file_path], check=True)
        elif platform.system() == 'Linux':
            subprocess.run(['xdg-open', text_file_path], check=True)
        else:
            print("Unsupported OS")
    else:
        print("Error: No PDF file selected.")

def show_confirmation_frame(pdf_path):
    ...
    yes_button = tk.Button(confirmation_frame, text="Yes", command=lambda: proceed(pdf_path))
